...
import { Button, Divider, Upload, Form, Input } from "antd";
import React, {
  useCallback,
  useEffect,
  useMemo,
...
const FormSolution = () => {
  ...
  const onFinish = async (values) => {
    ...
    try {
      if (isEdit) {
        await partnerHubSolutionService.updatePartnerHubSolution(id, newData);
      } else {
        await partnerHubSolutionService.createPartnerHubSolution(newData);
        form.resetFields();
        setImageUrl(null);
        setImageRes(null);
      }
  ...
  useEffect(() => {
    isEdit &&
      (async () => {
        try {
          const { data } =
            await partnerHubSolutionService.getPartnerHubSolutionById(id);
          setData(data.data);
