...
const FormSupport = () => {
  ...
  const onFinish = async (values) => {
    ...
    try {
      if (isEdit) {
        await partnerHubSupportService.updatePartnerHubSupport(id, newData);
      } else {
        await partnerHubSupportService.createPartnerHubSupport(newData);
        form.resetFields();
        setImageUrl(null);
        setImageRes(null);
      }
  ...
  useEffect(() => {
    isEdit &&
      (async () => {
        try {
          const { data } =
            await partnerHubSupportService.getPartnerHubSupportById(id);
          setData(data.data);
